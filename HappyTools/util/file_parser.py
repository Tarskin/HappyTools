...
def file_parser(master):
    try:
        if master.filename.suffix.lower().endswith('txt'):
            ...
            logging.getLogger(__name__).warning('Unsopported filetype for '
                                                + str(master.filename))
...
def arw_file_parser(master):
    chrom_data = []
    with Path(master.filename).open(encoding=master.settings.encoding) as fr:
        for line in fr:
            lines = line.split('\r')
            for sub_line in lines:
                if sub_line[0][0].isdigit() is False:
                    pass
                else:
                    line_chunks = sub_line.rstrip()
